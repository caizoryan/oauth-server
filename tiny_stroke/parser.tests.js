...
describe('parse', () => {
  ...
  it('parses multiple strokes', () => {
    ...
    assert.deepStrictEqual(result, [
      { color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100]] },
      { color: 'rgb(0,0,255)', brushSize: 4, points: [[5, 5]] }
    ]);
  });
...
describe('encode', () => {
  it('encodes a single stroke with one point', () => {
    const result = encode([{ color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100]] }]);
    assert.deepStrictEqual(result, ['25500000005000100']);
  });

  it('encodes a single stroke with multiple points', () => {
    const result = encode([{ color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100], [200, 200]] }]);
  ...
  it('pads single digit coordinates', () => {
    const result = encode([{ color: 'rgb(0,255,0)', brushSize: 1, points: [[1, 2]] }]);
    assert.deepStrictEqual(result, ['00025500001001002']);
  });

  it('pads double digit coordinates', () => {
    const result = encode([{ color: 'rgb(128,128,128)', brushSize: 10, points: [[10, 99]] }]);
    assert.deepStrictEqual(result, ['12812812810010099']);
  });

  it('handles maximum values', () => {
    const result = encode([{ color: 'rgb(255,255,255)', brushSize: 99, points: [[999, 999]] }]);
...
describe('parse and encode roundtrip', () => {
  it('roundtrips a single stroke with one point', () => {
    const original = [{ color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100]] }];
  ...
  it('roundtrips a single stroke with multiple points', () => {
    const original = [{ color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100], [200, 200], [50, 75]] }];
    const encoded = encode(original);
    const decoded = parse(encoded);
    assert.deepStrictEqual(decoded, original);
  });
  ...
  it('roundtrips multiple strokes', () => {
    const original = [
      { color: 'rgb(255,0,0)', brushSize: 5, points: [[0, 100], [150, 200]] },
      { color: 'rgb(0,0,255)', brushSize: 4, points: [[5, 5]] },
      { color: 'rgb(0,255,0)', brushSize: 10, points: [[100, 100], [200, 150], [300, 200]] }
    ];
    const encoded = encode(original);
    const decoded = parse(encoded);
    assert.deepStrictEqual(decoded, original);
  ...
  it('roundtrips with various color values', () => {
    const original = [
      { color: 'rgb(0,0,0)', brushSize: 1, points: [[0, 0]] },
      { color: 'rgb(255,255,255)', brushSize: 99, points: [[999, 999]] },
      { color: 'rgb(128,64,192)', brushSize: 50, points: [[123, 456]] }
    ];
    const encoded = encode(original);
    const decoded = parse(encoded);
    assert.deepStrictEqual(decoded, original);
